...
function tambah() {
	...
	newWindow('jenis_pengujian_add.php?preplid='+preplid+'&nama_pel='+nama_pel+'&nama_dep='+nama_dep, 'TambahJennisPengujian','439','208','resizable=1,scrollbars=0,status=0,toolbar=0')
}
...
function edit(replid) {
	newWindow('jenis_pengujian_edit.php?replid='+replid, 'UbahJenisPengujian','439','208','resizable=1,scrollbars=0,status=0,toolbar=0')
}
...
function cetak(id) {
	newWindow('jenis_pengujian_cetak.php?id='+id, 'CetakSemester','790','650','resizable=1,scrollbars=1,status=0,toolbar=0')
}
</script>
</head>
...
<?  OpenDb();
	$sql = "SELECT j.replid,j.jenisujian,j.idpelajaran,j.keterangan,p.replid,p.nama,p.departemen FROM jenisujian j, pelajaran p WHERE j.idpelajaran = '$id' AND j.idpelajaran = p.replid ORDER BY j.jenisujian";   
	$result = QueryDb($sql); 
