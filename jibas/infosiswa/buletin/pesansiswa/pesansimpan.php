...
OpenDb();
$sql="INSERT INTO jbsvcr.pesan SET tanggalpesan='".$tanggalpesan." ".$jampesan."', tanggaltampil='$tanggaltampil',judul='$judul',pesan='$pesan',nis='$idpengirim',idguru=NULL,kesiswa=1";
$result=QueryDb($sql);
CloseDb();
...
if ($file1!=""){
	...
	$namafile1=str_replace(" ","",$x1[0]);
		...
		OpenDb();
		$sql="INSERT INTO jbsvcr.lampiranpesan SET idpesan='$lastid', namafile='".$namafile1."-".$lastid.$ext1."', direktori='".$new_dir_db."'";
		$result=QueryDb($sql);
		CloseDb();
}
...
if ($file2!=""){
	...
	$namafile2=str_replace(" ","",$x2[0]);
		...
		OpenDb();
		$sql="INSERT INTO jbsvcr.lampiranpesan SET idpesan='$lastid', namafile='".$namafile2."-".$lastid.$ext2."', direktori='".$new_dir_db."'";
		$result=QueryDb($sql);
		CloseDb();
}
...
if ($file3!=""){
	...
	$namafile3=str_replace(" ","",$x3[0]);
		...
		OpenDb();
		$sql="INSERT INTO jbsvcr.lampiranpesan SET idpesan='$lastid', namafile='".$namafile3."-".$lastid.$ext3."', direktori='".$new_dir_db."'";
		$result=QueryDb($sql);
		CloseDb();
}
?>
