...
if (SI_USER_ID()=="adminsiswa" || SI_USER_ID()=="ADMINSISWA") {
...
parent.beritasekolah_header.lihat2('<?=$tgl[1]?>','<?=$tgl[2]?>');
</script>
<?
} elseif ($sender=="ubah"){
	OpenDb();
	...
	<?
	if (SI_USER_ID()=="adminsiswa" || SI_USER_ID()=="ADMINSISWA") {
	$sql18="UPDATE jbsvcr.beritasekolah SET judul='$judul', tanggal='$tanggal', jenisberita='$jenisberita', abstrak='$abstrak', isi='$isi', idpengirim='$idpengirim' WHERE replid='$replid'";
	} else {
	$sql18="UPDATE jbsvcr.beritasekolah SET judul='$judul', tanggal='$tanggal', abstrak='$abstrak', isi='$isi', idpengirim='$idpengirim' WHERE replid='$replid'";
	}
