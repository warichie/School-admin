...
class CDaftar{
    ...
    function Content(){
		...
		elseif ($this->kriteria=='tglpinjam')
			$sql = "SELECT * FROM pinjam WHERE status=1 AND tglpinjam BETWEEN '".MySqlDateFormat($this->tglAwal)."' AND '".MySqlDateFormat($this->tglAkhir)."' ORDER BY tglpinjam";
		elseif ($this->kriteria=='tglkembali')
			$sql = "SELECT * FROM pinjam WHERE status=1 AND tglkembali BETWEEN '".MySqlDateFormat($this->tglAwal)."' AND '".MySqlDateFormat($this->tglAkhir)."' ORDER BY tglpinjam";
		else
			$sql = "SELECT * FROM pinjam WHERE status=1 AND idanggota='$this->noanggota' ORDER BY tglpinjam";	
		//echo $sql;
		$result = QueryDb($sql);
