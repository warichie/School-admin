function OnLoad(){
	...
	sendRequestText('penilaian.ajax.list.php',ShowInfoGenList,'op=GetInfoGenList&Bln='+Bln+'&Thn='+Thn);
}
...
function SelectInfoGenList(replid){
	...
	sendRequestText('penilaian.ajax.list.php',ShowDetailInfoGenList,'op=GetDetailInfoGenList&replid='+replid);
}
...
function DeleteInfoGenList(ID){
	if (confirm('Are you sure want to delete seluruh SMS dalam info ini?')){
		...
		var Thn = document.getElementById('Year').value;
		ShowWait('InfoGenList');
		sendRequestText('penilaian.ajax.list.php',ShowInfoGenList,'op=DeleteInfoGenList&Bln='+Bln+'&Thn='+Thn+'&replid='+ID);
		document.getElementById('DetailInfoGenList').innerHTML = '';
	}
}
