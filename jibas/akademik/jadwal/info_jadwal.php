...
<tr>
    ...
    <tr height="25">
        ...
        <td align="center">
        <? if ($row['aktif']==1){ ?>
            <img src="../images/ico/aktif.png" onClick="setaktif(<?=$row['aktif']?>,<?=$row['replid']?>)" onMouseOver="showhint('Status Active', this, event, '80px')">
        <? } else { ?>
            <img src="../images/ico/nonaktif.png" onClick="setaktif(<?=$row['aktif']?>,<?=$row['replid']?>)" onMouseOver="showhint('Status Inactive', this, event, '80px')">
        <? } ?> 
		</td>
        <td align="center">
        	<a href="#" onClick="newWindow('info_jadwal_edit.php?replid=<?=$row['replid']?>',     'UbahInfoJadwal','390','250','resizable=1,scrollbars=1,status=0,toolbar=0')"><img src="../images/ico/ubah.png" border="0" onMouseOver="showhint('Edit Schedule Info', this, event, '80px')"></a>&nbsp;
