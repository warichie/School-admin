...
if ($op == "348328947234923") 
{
	...
	{
		// -------------------------------------------
		// Besar pembayaran student candidate telah terdata 
		// -------------------------------------------
		...
		{
			// total pembayaran yang dilakukan lebih besar from besar pembayaran yang diinput
		...
		if ($continue)
		{
			// ambil idjurnal and idtahunbuku on saat input besar pembayaran
		...
		if ($success && $continue)
		{
			// jika data from JIBAS keuangan <= 2.0 maka idtahunbuku_jtt == 0
			//   pastinya tidak akan sama dengan idtahunbuku 
			//   jadi create_adjustment = true;
			// jika data from JIBAS keuangan >= 2.1 maka idtahunbuku_jtt != 0
			...
			if ($create_adjustment)
			{
				...
				$tcicilan = date("Y-m-d");	
				$petugas = getUserName();
				$kjurnal = "Jurnal penyesuaian perubahan besar pembayaran $namapenerimaan student candidate $namasiswa ($nis)";
			...
			{
				// hanya terjadi untuk JIBAS Finance versi >= 2.1
				//   update jurnaldetail on saat pendataan besar pembayaran
	...
	{
		// ----------------------------------------
		// Data Collection Payment Oleh Student Candidate   
		// ----------------------------------------
		...
		$tcicilan = date("Y-m-d");	
		$petugas = getUserName();
		$keterangan = "Data Collection besar pembayaran $namapenerimaan student candidate $namasiswa ($nopendaftaran)";
...
// FIXED: 24 August 2010

// periksa apakah berasal from input JIBAS versi < 2.1
...
function validasi_besar() 
{
	...
	var isok = validateEmptyText('besar', 'Besarnya Payment') &&
		       validasiAngka() &&
		       validateMaxText('keterangan', 255, 'Info Besarnya Payment');
				  
	if (isok && idbesarjtt > 0)				  
		isok = isok && validateEmptyText('alasan','Alasan Data Changing');
...
function tambah() 
{	
	newWindow('pembayaranjttcalon_add.php?idpenerimaan=<?=$idpenerimaan?>&idkategori=<?=$idkategori?>&replid=<?=$replid?>&idtahunbuku=<?=$idtahunbuku?>', 'TambahPembayaran','405','390','resizable=1,scrollbars=1,status=0,toolbar=0')
}
...
<tr>
    ...
    <tr>
    	<td valign="top" width="265">
            ...
            <tr>
                <td valign="top" align="left" colspan="2">Info</td>
            </tr>
            <tr>
            	<td colspan="2"><textarea id="keterangan" name="keterangan" rows="3" cols="35" onKeyPress="return focusNext('simpan', event)" <? //$dis?> onfocus="panggil('keterangan')" style="width:225px; height:30px"><?=$keterangan ?></textarea>
            	</td>
            </tr>
<?			   if ($idbesarjtt > 0) { ?>
				<tr>
                <td valign="top" colspan="2" align="left"><strong>Reason perubahan data</strong></td>
            </tr>
            <tr>
...
<?			   } ?>               
           	<tr>
            	...
            	<input type="button" name="simpan" id="simpan" class="but" value="Save" onclick="this.disabled = true; ValidateSubmit();" style="width:100px" onfocus="panggil('simpan')"/>
            	</td>
        	</tr>
            </table>
            </form>
