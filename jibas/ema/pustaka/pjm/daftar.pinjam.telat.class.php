...
class CTelat{
    ...
    function Content(){
		...
		if ($this->kriteria=='all')
			$sql = "SELECT * FROM $db_name_perpus.pinjam WHERE status=1 AND tglkembali<'".$now."' ORDER BY tglpinjam";
		...
		elseif ($this->kriteria=='tglkembali')
			...
			$sql = "SELECT * FROM $db_name_perpus.pinjam WHERE status=1 AND idanggota='$this->noanggota' AND tglkembali<'".$now."' ORDER BY tglpinjam";
