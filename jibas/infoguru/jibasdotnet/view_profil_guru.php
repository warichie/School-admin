...
$r1=QueryDb("SELECT * FROM jbsvcr.profil WHERE nip<>'' AND nip<>'".$idguru."'");
$jumguru=@mysql_num_rows($r1);
$r2=QueryDb("SELECT * FROM jbsvcr.profil WHERE nis<>''");
$jumsiswa=@mysql_num_rows($r2);
$r3=QueryDb("SELECT * FROM jbsvcr.galerifoto WHERE idguru='".$idguru."'");
$jumfoto=@mysql_num_rows($r3);
$halfoto = ceil(@mysql_num_rows($r3)/(int)$barisfoto);
$r4=QueryDb("SELECT * FROM jbsvcr.komentar WHERE owner='".$idguru."'");
$jumkomentar=@mysql_num_rows($r4);
$totalkom = ceil(@mysql_num_rows($r4)/(int)$bariskomentar);
$r5=QueryDb("SELECT * FROM jbsvcr.buletin WHERE idpengirim='".$idguru."'");
$jumkarya=@mysql_num_rows($r5);
$halkarya = ceil(@mysql_num_rows($r5)/(int)$bariskarya);
//
$res_prof=QueryDb("SELECT * FROM jbsvcr.profil WHERE nip='".$idguru."'");
$row_prof=@mysql_fetch_array($res_prof);

$res_peg=QueryDb("SELECT * FROM jbssdm.pegawai WHERE nip='".$idguru."'");
$row_peg=@mysql_fetch_array($res_peg);

$res_fot=QueryDb("SELECT * FROM jbsvcr.galerifoto WHERE idguru='".$idguru."' ORDER BY replid DESC LIMIT 0,4");

$res_kar=QueryDb("SELECT * FROM jbsvcr.buletin WHERE idpengirim='".$idguru."' ORDER BY replid DESC LIMIT 0,".$bariskarya);

$res_guru=QueryDb("SELECT * FROM jbsvcr.profil WHERE nip<>'".$idguru."' AND nip<>'' ORDER BY replid DESC LIMIT 0,3");

$res_siswa=QueryDb("SELECT * FROM jbsvcr.profil WHERE nis<>'' ORDER BY replid DESC LIMIT 0,3");

$res_kom=QueryDb("SELECT DATE_FORMAT(tanggal, '%Y-%m-%d') as tanggal,DATE_FORMAT(tanggal, '%H:%i:%s') as jam, komentar, nip, replid FROM jbsvcr.komentar WHERE owner='".$idguru."' ORDER BY replid DESC LIMIT ".(int)$page*(int)$bariskomentar.",".$bariskomentar);

$op="";
if (isset($_REQUEST[op]))
	$op=trim($_REQUEST[op]);
if ($op=="komensimpan"){
	QueryDb("INSERT INTO jbsvcr.komentar SET owner='".$idguru."', nip='$_REQUEST[sender]', komentar='$_REQUEST[komentar]', tanggal=now()");
...
function change_page(page,replid) {
	document.location.href="view_profil_guru.php?page="+page+"&replid="+replid;
}
...
<form action="view_profil_guru.php" method="GET">
            ...
            <td width="305" valign="top">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                    ...
                    <td height="25" align="right" valign="top">&nbsp;<a href="#" onClick="document.location.href='viewallguru.php?id=<?=$idguru?>'"><em>View All <span class="style4">(<?=$jumguru?>)</span> ...</em></a></td>
                  </tr>
                  <tr>
                    <td colspan="3">
                        <table width="100%" border="0" cellspacing="0">
                          <tr>
							...
							?>
							<td align="center" style="cursor:pointer;" onClick="document.location.href='view_profil_guru.php?replid=<?=$row_gr[replid]?>';"><img src="../library/gambar.php?replid=<?=$row_gr[replid]?>&table=jbsvcr.profil" width="50"><br><?=$row_pg[panggilan]?></td>
                            <?
							}
							?>
                          </tr>
                  ...
                  <tr>
                    ...
                    <td height="25" valign="top">&nbsp;</td>
                    <td height="25" align="right" valign="top">&nbsp;<a href="#" onClick="document.location.href='viewallsiswa.php?id=<?=$idguru?>'"><em>View All <span class="style4">(<?=$jumsiswa?>)</span> ...</em></a></td>
                  </tr>
                  <tr>
                    <td colspan="3">
                    	<table width="100%" border="0" cellspacing="0">
                  ...
                  <tr>
                    ...
                    <td align="right"><?
						...
						?>
						Page : 
						<input <?=$disback?> type="button" class="but" title="Previous" name="back" value="<" onClick="change_page('<?=(int)$page-1?>','<?=$replid?>')" onMouseOver="showhint('Previous', this, event, '75px')">
						<?=$page+1?>
						<input <?=$disnext?> type="button" class="but" name="next" title="Next" value=">" onClick="change_page('<?=(int)$page+1?>','<?=$replid?>')" onMouseOver="showhint('Next', this, event, '75px')">
						<? } ?>					</td>
                  </tr>
                </table>            </td>
			<td width="20">&nbsp;</td>
            <td valign="top"><table width="100%" border="0" cellspacing="0">
              ...
              <? 
				   if ($op=="viewallphoto"){
					 for ($i=0;$i<=$halfoto;$i++){ ?>
			  <tr><?		
					$res_foto=QueryDb("SELECT * FROM jbsvcr.galerifoto WHERE idguru='$idguru' ORDER BY replid DESC LIMIT ".(int)$i*(int)$barisfoto.",".$barisfoto);
						while ($row_foto=@mysql_fetch_array($res_foto)){
					?>
                <td width="25%"><div align="center"><a href="<?=$WEB_GALLERY_DIR?>photos/<?=$row_foto[filename]?>" rel="lytebox[vacation]"  >
			  ...
			  <?
				while ($row_fot=@mysql_fetch_array($res_fot)){
					?>
                <td width="25%"><div align="center"><a href="<?=$WEB_GALLERY_DIR?>photos/<?=$row_fot[filename]?>" rel="lytebox[vacation]"  ><img   src="<?=$WEB_GALLERY_DIR?>thumbnail/<?=$row_fot[filename]?>" width="80" style="cursor:pointer;"><br>
			 ...
			 if ($opkarya=="viewallkarya"){
			  ...
			  <tr><?		
					$res_karya=QueryDb("SELECT * FROM jbsvcr.buletin WHERE idpengirim='$idguru' ORDER BY tanggalinput DESC LIMIT ".(int)$i*(int)$bariskarya.",".$bariskarya);
