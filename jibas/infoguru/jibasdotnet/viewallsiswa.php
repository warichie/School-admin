...
$rs=QueryDb("SELECT * FROM jbsvcr.profil WHERE nip='".$id."'");
//echo "SELECT * FROM jbsvcr.profil WHERE nip='".SI_USER_ID()."'";
$numguru = @mysql_num_rows($rs);
if ($numguru>0){
$rw=@mysql_fetch_array($rs);
$rep=$rw[replid];
} else {
	$rss=QueryDb("SELECT * FROM jbsvcr.profil WHERE nis='".$id."'");
	//echo "SELECT * FROM jbsvcr.profil WHERE nip='".SI_USER_ID()."'";
	$numsiswa = @mysql_num_rows($rss);
	$rws=@mysql_fetch_array($rss);
	$rep=$rws[replid];
}

//echo $repguru;
$r1=QueryDb("SELECT * FROM jbsvcr.profil WHERE nis<>'' AND nis<>'".$id."' ORDER BY replid ");
...
function change_page(page,id) {
	document.location.href="viewallsiswa.php?page="+page+"&id="+id;
}
</script>
</head>
<body style="background-attachment:fixed;"  background="../library/gambarlatar.php?replid=<?=$rep?>&table=jbsvcr.profil"  onLoad="rbInit();"  onResize="rbResize()" bgcolor="#000000" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
<script>
  ...
  <tr>
    ...
    <td style="background-image:url(../images_slice/bg_profil80_08.png); background-repeat:repeat;" valign="top">
        <table width="100%" border="0" cellspacing="0">
          <tr>
			<td>
            <div align="right"><span class="style8">Student List</span> 
            </div>
			...
			<?
				$xx=20;
				$s = "SELECT * FROM jbsvcr.profil WHERE nis<>'' AND nis<>'".$id."' LIMIT ".(int)$page*(int)$xx.",".(int)$xx;
				$rs2 = QueryDb($s);	
				$x=0;
				while ($rw=@mysql_fetch_array($rs2)){
					...
					$panggilan="";
					$rs3 = QueryDb("SELECT panggilan FROM jbsakad.siswa WHERE nis='$rw[nis]'");
					$rw3=@mysql_fetch_array($rs3);
					$panggilan=$rw3[panggilan];
					echo "<td width='20%' valign='top' ><img title='".$rw[nama]."' style='cursor:pointer;' onclick=\"document.location.href='view_profil_siswa.php?replid=".$rw[replid]."'\" src='../library/gambar.php?replid=$rw[replid]&table=jbsvcr.profil' width='75' align='left'>".
						 "<span class='style5'><a style='text-decoration:none;' href='view_profil_siswa.php?replid=".$rw[replid]."'>".$panggilan."</a></span><br>".
						 "<span class='style7'>$rw[email]</span><br>".
						 "<span class='style7'>$rw[tentang]</span><br>".
						 "</td>";
					if ($x+1%5==0){
						echo "</tr>";
					}
			...
			?>  
						...
						?>
						<a style="text-decoration:none;" href="viewallguru.php?page=<?=(int)$hal-1?>&id=<?=$id?>"><?=$f.$hal.$r?></a>
						<?
						}
						?>
