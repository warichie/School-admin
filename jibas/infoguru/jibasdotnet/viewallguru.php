...
//echo $repguru;
$r1=QueryDb("SELECT * FROM jbsvcr.profil WHERE nip<>'' AND nip<>'".$id."' ORDER BY replid ");
...
function change_page(page,id) {
	document.location.href="viewallguru.php?page="+page+"&id="+id;
}
</script>
</head>
    ...
    <td style="background-image:url(../images_slice/bg_profil80_08.png); background-repeat:repeat;" valign="top">
        <table width="100%" border="0" cellspacing="0">
          <tr>
			...
			<?
				$xx=20;
				$s = "SELECT * FROM jbsvcr.profil WHERE nip<>'' AND nip<>'".$id."' LIMIT ".(int)$page*(int)$xx.",".(int)$xx;
				$rs2 = QueryDb($s);	
				$x=0;
				while ($rw=@mysql_fetch_array($rs2)){
					...
					echo "<td width='20%' valign='top' ><img title='".$rw[nama]."' style='cursor:pointer;' onclick=\"document.location.href='view_profil_guru.php?replid=".$rw[replid]."'\" src='../library/gambar.php?replid=$rw[replid]&table=jbsvcr.profil' width='75' align='left'>".
						 "<span class='style5'><a style='text-decoration:none;' href='view_profil_guru.php?replid=".$rw[replid]."'>".$panggilan."</a></span><br>".
