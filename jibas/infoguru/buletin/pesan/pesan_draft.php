...
function bacapesan(replid){
	document.location.href="pesanbaca_draft.php?replid="+replid;
}
...
<table width="100%" border="0" cellspacing="0">
  ...
  $sql1="SELECT d.replid as replid,d.judul as judul, d.pesan as pesan, d.idpengirim as nip, DATE_FORMAT(d.tanggalpesan, '%e %b %Y') as tanggal, TIME_FORMAT(d.tanggalpesan, '%H:%i') as waktu FROM jbsvcr.draft d WHERE  d.idpemilik='".SI_USER_ID()."' ORDER BY replid LIMIT ".(int)$page*(int)$varbaris.",$varbaris";
  ...
  while ($row1=@mysql_fetch_array($result1)){
	  $sql2 = "SELECT nama FROM jbssdm.pegawai WHERE nip='$row1[nip]'";
	  $result2 = QueryDb($sql2);
	  $num2 = @mysql_num_rows($result2);
	  if ($num2>0){
		$row2 = @mysql_fetch_array($result2);
	  } else {
		$sql3 = "SELECT nama FROM jbsakad.siswa WHERE nis='$row1[nip]'";
