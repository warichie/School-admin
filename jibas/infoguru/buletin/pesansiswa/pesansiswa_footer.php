...
if ($op=="f3fxxa7svys774l3067den747hhd783uu83"){//Mindahin ke draft
	...
	while ($x<=$numdel){
		...
		//$olddir=str_replace($WEB_UPLOAD_DIR,$UPLOAD_DIR,$row5['direktori']);
		$new_olddir = str_replace("/","\\",$row5['direktori']);
		//echo $UPLOAD_DIR.$new_olddir;
		copy($UPLOAD_DIR."pesan\\".$new_olddir.$row5['namafile'], $UPLOAD_DIR."pesan\\".$new_olddir.SI_USER_ID()."-".$row5['namafile']);
		//echo $row5['direktori'].$row5['namafile'];
		$sql6="INSERT INTO jbsvcr.lampirandraft SET idpesan=$lastid,direktori='".$dir_thn . $dir_bln."/',namafile='".SI_USER_ID()."-".$row5['namafile']."'";
