...
function bacapesan(replid){
	document.location.href="pesansiswabaca_terkirim.php?replid="+replid;
}
...
<table width="100%" border="0" cellspacing="0">
  <tr>
  <? OpenDb();
  $sql_tot="SELECT * FROM jbsvcr.pesanterkirim pt, jbsvcr.pesan p WHERE p.idguru='".SI_USER_ID()."' AND p.kesiswa=1 AND pt.idpesan=p.replid";
  ...
  $sql1="SELECT pt.replid as replid_tkrm, pg.replid as replid, pg.judul as judul, DATE_FORMAT(pg.tanggalpesan, '%e %b %Y') as tanggal, TIME_FORMAT(pg.tanggalpesan, '%H:%i') as waktu FROM jbsvcr.pesan pg, jbsvcr.pesanterkirim pt WHERE pg.idguru='".SI_USER_ID()."' AND kesiswa=1 AND pt.idpesan=pg.replid ORDER BY pg.tanggalpesan DESC, pg.replid DESC LIMIT ".(int)$page*(int)$varbaris.",$varbaris";
