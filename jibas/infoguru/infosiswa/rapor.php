...
<tr>
    ...
    <tr>
        ...
        <td>
        <select name="tahunajaran" id="tahunajaran" onChange="change_tahunajaran(4)" style="width:150px">
    ...
    <tr>
        <td width="19%"><strong>Class History</strong></td>
        <td>
        ...
        <? 	$sql = "SELECT * FROM semester WHERE departemen = '$depart' ORDER BY replid";			
			...
			while ($row = @mysql_fetch_array($result)) 
			{
				if ($semester == "") 
					$semester = $row['replid'];	?>
				<option value="<?=$row['replid']?>" <?=IntIsSelected($row['replid'], $semester) ?> > <?=$row['semester']?> </option>
		<? 	} ?>
    	</select>    
		</td>
    </tr>
